package com.salor.ventgo.ui.activity.stock_opname_stock.scanner
...
import com.salor.ventgo.obj.riwayat_stock_opname_stock.scan.ScanStockOpnameStockDetail
...
class ScannerActivity : BaseActivity(), DecoratedBarcodeView.TorchListener {
    ...
    var isAddStockOpname: Boolean = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (IntentIntegrator.isScan) {
            ...
            capture = ScannerActivity.CaptureManager(this, barcodeScannerView!!, dataRiwayat)
...
//            closeAndFinish()
        ...
        private fun userScan(code: String) {
            ...
            val call = service.stockOpnameStockScan(code)
                    ...
                    if (response.isSuccessful) {
                        try {
                            ...
                            if (api_status == Cons.INT_STATUS) {
                                ...
                                val json_scan = json.getJSONObject(Cons.ITEMS_DATA)

                                val scanStock = Gson().fromJson(json_scan.toString(), ScanStockOpnameStockDetail::class.java)

                                activity.dialogInputStockQuantity(scanStock)
...
//        fun dialogInputStockQuantity(scanData: ScanStockOpnameStockDetail) {
//
//            try {
//
...
//                })
//
//                var str_selected_tipe_quantity: String = ""
//                var qty: String = ""
//
//                val listSpinnerTipeQuantity: ArrayList<String> = ArrayList()
//
//                for (i in 0 until scanData.selectQtyType.size) {
//
//                    val item = scanData.selectQtyType[i]
//
//                    listSpinnerTipeQuantity.add(item.qtyType)
//
//                }
//
...
//
//                        See.logE("str_selected_tipe_quantity", "" + str_selected_tipe_quantity)
//
//                    }
//
...
//
//                    if (qty == "") {
//                        etQty.error = activity.resources.getString(R.string.label_form_wajib_diisi)
//                    } else if (str_selected_tipe_quantity == "") {
//
//                        See.toast(activity, activity.resources.getString(R.string.label_isikan_tipe_quantity_dahulu))
//
//                    } else {
//
//                        pDialog.dismiss()
//                        stockOpnameAdd(scanData, str_selected_tipe_quantity, qty)
//
//                    }
//
//
...
//        }
        ...
        private fun stockOpnameAdd(scanDataOpname: ScanStockOpnameStockDetail, qtyType: String, qty: String) {
            pLoading.showLoading(activity.resources.getString(R.string.label_loading_title_dialog), false)
            val service = ApiClient.getClient()
            val gson = Gson()

            val id_item = scanDataOpname.id
            val id_cms = scanDataOpname.idCmsUsers

            val call = service.stockOpnameStockAddItem(id_item, id_cms, qtyType, qty, scanDataOpname.sku, scanDataOpname.id)

            See.logE("id_item", "" + id_item)
            See.logE("id_cms", "" + id_cms)
            See.logE("id_stockopname", "" + scanDataOpname.id)
                ...
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    ...
                    if (response.isSuccessful) {
                        try {
                            ...
                            See.logE("respon_scan_add", respon)
                            ...
                            if (api_status == Cons.INT_STATUS) {
                                ...
                                activity.isAddStockOpname = true

                                dialogScanSuccess(scanDataOpname.sku)
        ...
        fun dialogScanSuccess(sku: String) {
    ...
    private fun userScan(code: String) {
        pLoading.showLoading(this@ScannerActivity.resources.getString(R.string.label_loading_title_dialog), false)
        val service = ApiClient.getClient()
        val gson = Gson()
                ...
                if (response.isSuccessful) {
                    try {
                        ...
                        if (api_status == Cons.INT_STATUS) {
                            ...
                            dialogInputStockQuantity(scanStock)
